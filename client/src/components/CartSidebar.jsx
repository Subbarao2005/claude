              ...
              {validItems.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center py-20">
                  <div className="w-32 h-32 bg-stone-50 rounded-[3rem] flex items-center justify-center mb-10 group">
                    <ShoppingCart size={48} className="text-stone-200 group-hover:text-primary transition-colors duration-500" />
                  </div>
                  <h3 className="text-3xl font-playfair font-bold text-stone-900 mb-4">Your bag is empty</h3>
                  <p className="text-stone-400 font-medium mb-10 max-w-[240px] mx-auto">
                    Add some delicious desserts to start your sweet journey!
                  </p>
                  <button
                ...
                validItems.map((item, index) => {
                  const productId = item?.product?._id || item?._id || `item-${index}`;
                    ...
                    >
                      <div className="w-20 h-20 bg-stone-50 rounded-2xl overflow-hidden flex-shrink-0 border border-stone-50">
                        {image ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-stone-200 bg-gradient-to-br from-stone-50 to-stone-100">
                      ...
                      <div className="flex-grow flex flex-col justify-center min-w-0">
                        ...
                        <p className="text-stone-400 text-xs font-bold mb-3">Unit Price: {formatCurrency(price)}</p>
                        ...
                        <div className="flex items-center justify-between mt-auto">
                          ...
                          <span className="font-bold text-primary">{formatCurrency(price * quantity)}</span>
                        </div>
                      </div>
                    </motion.div>
                  );
              ...
              <div className="p-8 bg-stone-50 border-t border-stone-100 space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center text-stone-900">
                    ...
                    <span className="text-2xl font-bold">{formatCurrency(cartTotal || 0)}</span>
                  </div>
                </div>
